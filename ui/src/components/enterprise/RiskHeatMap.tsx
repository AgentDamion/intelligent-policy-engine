...
import type { HeatMap } from "../../app/enterprise/types";
...
export function RiskHeatMap({
  ...
  const cellMap = new Map(
    data.matrix.map((m) => [key(m.partner, m.category), m])
  );
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-separate border-spacing-0">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-white p-2 text-left text-xs font-medium text-slate-500">
                {/* spacer for partner labels */}
              </th>
              {data.categories.map((c) => (
                <th
                  key={c}
                  className="p-2 text-left text-xs font-medium text-slate-500"
                >
                  {c}
                </th>
              ))}
          ...
          <tbody>
            {data.partners.map((p) => (
              <tr key={p}>
                <th className="sticky left-0 z-10 bg-white py-1.5 pr-4 text-left text-xs font-medium text-slate-600">
                  {p}
                </th>
                {data.categories.map((c) => {
