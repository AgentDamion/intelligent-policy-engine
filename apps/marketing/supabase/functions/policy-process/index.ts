...
async function processPolicy(document: any, enterpriseId: string, options: any = {}): Promise<any> {
  ...
  try {
    ...
    const finalOutcome = aiResult.result.decision === 'approve' ? 'approved' : 
                        aiResult.result.decision === 'reject' ? 'rejected' : 'needs_review';
...
function getFallbackPolicyResult(document: any, enterpriseId: string, options: any): any {
