...
🔗 API base: ${baseUrl}`);
  ...
  try {
    ...
    const { count, raw } = await fetchPolicyTemplates(baseUrl);
    console.log(`   ✅ /policy-templates OK (count=${count})`);
    if (count === 0) {
      console.log('   ⚠️  No templates found. You may want to seed templates:');
      console.log('      - node database/seed-policy-templates.js');
    }
  } catch (err) {
    ok = false;
    console.error('   ❌ /policy-templates failed:', err.response?.status, err.message);
  }
