...
-- Policy 3: Users with 'contract_viewer' or 'finance' roles can view contracts
-- (This requires the user_roles system from the context)
CREATE TYPE IF NOT EXISTS public.app_role AS ENUM ('admin', 'user', 'contract_viewer', 'finance', 'legal', 'procurement');
...
-- Policy 4: Users with contract-related roles can view organization contracts
CREATE POLICY "Contract role users can view organization contracts" 
ON public.contracts 
FOR SELECT 
USING (
  user_has_role(auth.uid(), 'contract_viewer', organization_id) OR
  user_has_role(auth.uid(), 'finance', organization_id) OR
  user_has_role(auth.uid(), 'legal', organization_id) OR
  user_has_role(auth.uid(), 'procurement', organization_id)
);
