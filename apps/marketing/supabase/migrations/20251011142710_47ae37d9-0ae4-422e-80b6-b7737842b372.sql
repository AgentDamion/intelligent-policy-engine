-- Critical Security Migration: Enable RLS and Fix Search Paths (FINAL SIMPLIFIED)
-- This migration addresses all remaining security issues before beta launch

-- ============================================================================
-- PART 1: Enable RLS on 6 tables that are currently unprotected
-- ============================================================================

-- 1. exports_log table (references run_id -> sandbox_runs -> workspace_id)
...
-- 2. governance_events table (only has workspace_id, no enterprise_id)
...
-- 3. platform_document_syncs table (service role only for now)
...
-- 4. sandbox_approvals table (references run_id -> sandbox_runs -> workspace_id)
...
-- 5. sandbox_controls table (references run_id -> sandbox_runs -> workspace_id)
...
-- 6. sandbox_runs table (has workspace_id directly)
...
-- Fix can_enterprise_add_partner
...
-- Fix get_enterprise_partner_count
...
-- Fix is_admin_of_enterprise_or_workspace (2-parameter version)
