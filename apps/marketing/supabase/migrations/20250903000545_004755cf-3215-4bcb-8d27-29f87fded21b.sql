-- Continue with RLS setup and missing parts, skipping existing enum
...
-- RLS Policies for scores (inherit from submissions)
...
-- Audit events are append-only - no UPDATE/DELETE policies
...
-- Note: Adding to publication handled via Supabase dashboard

-- Generate unique token function for invitations (if not exists)
