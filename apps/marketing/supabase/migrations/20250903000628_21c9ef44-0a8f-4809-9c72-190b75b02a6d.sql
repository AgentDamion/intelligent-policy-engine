-- Create missing enums first (skip existing ones)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.policy_distributions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Add foreign key constraints that reference decisions (only if they don't exist)
DO $$ 
BEGIN
    -- Add decision_id foreign key to submissions if not exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_submissions_decision_id'
    ) THEN
    ...
    -- Add decision_id foreign key to submission_items if not exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_submission_items_decision_id'
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_policies_enterprise_id ON public.policies(enterprise_id);
...
CREATE INDEX IF NOT EXISTS idx_decisions_submission_item_id ON public.decisions(submission_item_id);
CREATE INDEX IF NOT EXISTS idx_evidence_submission_item_id ON public.evidence(submission_item_id);
