-- Enhance approval_workflows table with enterprise-grade features for collab-lite
-- Add SLA tracking, bottleneck detection, and assignment routing

-- Add new columns for enhanced approval pipeline
...
ADD COLUMN IF NOT EXISTS workflow_template text DEFAULT 'standard',
ADD COLUMN IF NOT EXISTS auto_assignment_rules jsonb DEFAULT '{}',
ADD COLUMN IF NOT EXISTS escalation_rules jsonb DEFAULT '{}',
ADD COLUMN IF NOT EXISTS stage_history jsonb DEFAULT '[]';

-- Update estimated_completion calculation to be based on SLA
UPDATE approval_workflows 
SET estimated_completion = COALESCE(started_at, created_at) + (sla_hours || ' hours')::interval
WHERE estimated_completion IS NULL;

-- Create function to auto-assign based on policy content and user expertise
...
-- Create function to detect bottlenecks and trigger escalations
...
-- Create trigger to automatically update due dates when workflows are created/updated
...
-- Create the trigger
DROP TRIGGER IF EXISTS trigger_update_workflow_due_date ON approval_workflows;
CREATE TRIGGER trigger_update_workflow_due_date
  BEFORE INSERT OR UPDATE ON approval_workflows
  FOR EACH ROW
  EXECUTE FUNCTION update_workflow_due_date();

-- Create index for performance on workflow queries
CREATE INDEX IF NOT EXISTS idx_approval_workflows_enterprise_status 
ON approval_workflows(enterprise_id, current_stage, bottleneck_detected);

CREATE INDEX IF NOT EXISTS idx_approval_workflows_due_date 
ON approval_workflows(due_date) WHERE due_date IS NOT NULL;