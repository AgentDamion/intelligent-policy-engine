...
import type { Overview, HeatMap, MetaLoopLatest, Approval, TimelineItem, PartnerHealth } from './types'
...
export function useEnterpriseDashboard() {
  const [filters, setFilters] = useState<Filters>({ unit: 'all', window: '7d', risk: 'all' })
  ...
  async function load() {
    try {
      ...
      const [ov, hm, ml, ap, tl, ph] = await Promise.all([
        ...
        get<Approval[]>(`/api/approvals${buildQuery({ partner: filters.partner, category: filters.category })}`),
  ...
  async function bulkApprove(ids: string[], action: 'approve' | 'request_changes' | 'assign') {
    try {
      ...
      const ap = await get<Approval[]>(`/api/approvals${buildQuery({ partner: filters.partner, category: filters.category })}`)
      setApprovals(ap)
    } catch (error: any) {
  ...
  return { filters, setFilters, overview, heat, intel, approvals, timeline, partners, loading, err, onHeatSelect, routeRecToReview, bulkApprove, reload: load }
}
