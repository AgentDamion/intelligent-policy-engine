const fs = require('fs');
// Prefer `.env.local` for local development (commonly contains DATABASE_URL, etc.)
require('dotenv').config({
  path: process.env.DOTENV_PATH || (fs.existsSync('.env.local') ? '.env.local' : '.env'),
});
...
// Feedback loop boot can pull in optional agent modules. Don't crash the API server if an agent is misconfigured.
try {
  require('./core/feedback-loop');
} catch (e) {
  console.warn('⚠️  Skipping feedback-loop initialization (non-blocking):', e?.message || e);
}
