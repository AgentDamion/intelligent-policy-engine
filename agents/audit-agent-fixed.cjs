/**
 * Audit Agent - Comprehensive Compliance Audit Trail System (Fixed Version)
 * 
 * Features:
...
class AuditAgent {
    constructor() {
        this.auditLog = [];
        this.completedSessions = []; // Store complete sessions separately
    ...
    completeAuditSession(finalDecision, totalProcessingTime) {
        ...
        // Add session summary to audit log
        this.auditLog.push({
    ...
    generateComplianceReport(sessionId = null) {
        ...
        const report = {
            ...
            summary: {
                ...
                total_decisions: sessions.reduce((sum, session) => sum + (session.audit_entries?.length || 0), 0),
            ...
            sessions: sessions.map(session => ({
                ...
                decision_count: session.audit_entries?.length || 0,
    ...
    getSessionById(sessionId) {
        return this.completedSessions.find(session => session.session_id === sessionId);
    }
    ...
    calculateAverageProcessingTime(sessions) {
        const total = sessions.reduce((sum, s) => sum + (s.total_processing_time || 0), 0);
        return sessions.length > 0 ? (total / sessions.length).toFixed(0) : 0;
    }

    countHighRiskDecisions(sessions) {
        return sessions.reduce((count, session) => {
            return count + (session.audit_entries?.filter(entry => entry.risk_level === 'high').length || 0);
        }, 0);
    }
...
function testAuditAgent() {
    ...
    console.log('📋 Testing Audit Agent (Fixed Version)...\n');
    ...
    // Simulate Context Agent decision
    const contextOutput = {
        urgency: { level: 0.3, emotionalState: "calm" },
        context: { inferredType: "creative_campaign", confidence: 0.7 },
        clarification: { question: "Which specific campaign is this for?" },
        recommendations: [{ priority: "medium", action: "Proceed with caution" }],
        nextSteps: ["Review brand guidelines", "Coordinate with clients"]
    };
    
    auditAgent.logContextDecision(contextOutput);
    
    // Simulate Policy Agent decision
    const policyOutput = {
        decision: { status: "approved", type: "conditional_approval" },
        risk: { score: 0.6, level: "medium", factors: ["Multi-client scenario"] },
        conditions: { guardrails: { content_review: true } },
        monitoring: { requirements: { usage_tracking: true } },
        next_steps: ["Implement guardrails", "Set up monitoring"]
    };
    
    auditAgent.logPolicyDecision(policyOutput, contextOutput);
    
    // Simulate Negotiation Agent decision
    const negotiationOutput = {
        clients: { count: 3, names: ["pfizer", "novartis", "roche"] },
        relationships: { competitors: [{ client1: "pfizer", client2: "novartis" }] },
        conflicts: { total: 3 },
        solution: { approach: "compromise", feasibility: "feasible" },
        decision: { status: "conditional_approval_with_escalation" },
        decision: { next_steps: ["Escalate to management"] }
    };
    
    auditAgent.logNegotiationDecision(negotiationOutput, contextOutput, policyOutput);
    
    // Complete session
    const finalDecision = {
        status: "conditional_approval_with_escalation",
        reasoning: "Complex multi-client scenario requires escalation"
    };
    
    const completedSession = auditAgent.completeAuditSession(finalDecision, 150);
    
    // Generate compliance report
    const report = auditAgent.generateComplianceReport(sessionId);
    
    console.log('📊 COMPLIANCE REPORT:');
    console.log(JSON.stringify(report, null, 2));
    
    // Search audit logs
    const searchResults = auditAgent.searchAuditLogs({ agent: 'negotiation' });
    console.log(`\n🔍 Search Results (Negotiation Agent): ${searchResults.length} entries`);
    
    // Export audit logs
    const csvExport = auditAgent.exportAuditLogs('csv');
    console.log(`\n📄 CSV Export (first 200 chars): ${csvExport.substring(0, 200)}...`);
}

// Export for use in other modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { AuditAgent, testAuditAgent };
}
