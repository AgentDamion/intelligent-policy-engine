...
import type { Approval } from "../../app/enterprise/types";
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <BulkButton
            disabled={!ids.length}
            label="Approve"
            onClick={() => onBulk(ids, "approve")}
            variant="primary"
          />
          <BulkButton
            disabled={!ids.length}
            label="Request Changes"
            onClick={() => onBulk(ids, "request_changes")}
          />
          <BulkButton
            disabled={!ids.length}
            label="Assign"
            onClick={() => onBulk(ids, "assign")}
          />
        </div>
      </div>
...
function BulkButton({
  ...
  const base =
    "rounded-lg px-3 py-1.5 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 transition";
  const styles =
    variant === "primary"
  ...
  return (
    <div className="flex h-40 items-center justify-center rounded-md border border-dashed border-slate-200">
      <div className="text-sm text-slate-500">
        No approvals right now — great job staying ahead!
      </div>
    </div>
  );
}
